...
boolCalc (Reln cOp expr1 expr2) = do
  c1 <- intCalc expr1
  c2 <- intCalc expr2
  return ((fromMaybe (error "This should never happen") (lookup cOp relnOps)) c1 c2)
boolCalc (BBin op expr1 expr2) =  do
  c1 <- boolCalc expr1
  c2 <- boolCalc expr2
  return ((fromMaybe (error "This should never happen") (lookup op bBinOps)) c1 c2)
boolCalc (BUn op expr) = do
  c1 <- boolCalc expr
  return ((fromMaybe (error "This should never happen") (lookup op bUnOps)) c1) 
...
intCalc (ABin op expr1 expr2) = do
 c1 <- intCalc expr1
 c2 <- intCalc expr2
 if c2 == 0 && op == Div then error "Cannot Divide by Zero" else
...
intCalc (AUn op expr) = do
  c1 <- intCalc expr
  return ((fromMaybe (error "This should never happen") (lookup op aUnOps)) c1)
